...
-- rediscli:lockProcess(GAMENAME..":halllock",function ()
    ...
    if rule then
        local col2 = db:get_col("ranktotalinnings")
        local cursor=col2:find({},nil,1000000)

        ret = ret or {}

        local tmsort = cursor:sort({count=-1})

        local result = {}
        result.data={}
        for index, item in pairs(tmsort) do
            -- local name = rediscli:hget(keysutils.get_model_key("user",item._id),"name")
            -- local uid1  = rediscli:hget(keysutils.get_model_key("user",item._id),"uid")
            -- local avatar  = rediscli:hget(keysutils.get_model_key("user",item._id),"avatar")
            if item['uid'] then
