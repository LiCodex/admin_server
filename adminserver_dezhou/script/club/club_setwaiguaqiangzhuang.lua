...
local uid1=args.uid
local daili1 =args.waiguaqiangzhuang
...
if daili > 0 then
    red:hset(keysutils.get_model_key("user",uid), 'waiguaqiangzhuang',0)
    result.code = 20000
    result.data = '删除成功'
else
    red:hset(keysutils.get_model_key("user",uid), 'waiguaqiangzhuang',1)
