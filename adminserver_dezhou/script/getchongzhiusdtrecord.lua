...
-- local row = {_id=id,coin=count,uid=uid,usdt=curusdt,wallet=wallet,time=os.time()}
-- 计算该玩家总充值额
local totalmoney = 0
if not err then
	for index, item in pairs(ret) do
		table.dumpdebug(item,'_________________item_________')
		if item['uid'] then
	    ...
	    local ok ,v = ptonumber(item.usdt)
	    if ok then
