...
local xing = {"赵", "钱", "孙", "李", "周", "吴", "郑", "王", "冯", "陈", "褚", "卫", "蒋", "沈", "韩", "杨", "朱", "秦", "尤", "许",
...
function rndname.rnd(count,gender)
    count = math.max(2,count)
    gender = gender or 1
    local characters = {}
    local r = math.random(1,#xing)
    table.insert(characters,xing[r])
    for i=1,count-1 do
        if gender==1 then
            local r = math.random(1,utf8.len(boy))
            table.insert(characters,utf8.sub(boy,r,r))
        else
            local r = math.random(1,utf8.len(girl))
            table.insert(characters,utf8.sub(girl,r,r))
        end
    end
